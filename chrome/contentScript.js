chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
  ...
  chrome.runtime.sendMessage({
    ...
    if (response && response.success) {
      console.log('Window created successfully via background script');
      sendResponse({ success: true });
    } else {
      console.log('Background script window creation failed, falling back to window.open');
      tryWindowOpen();
    }
  });
  ...
  function tryWindowOpen() {
    ...
    console.log('Attempting direct window.open for Chrome');
    const popup = window.open(window.location.href, windowName, windowFeatures);
    
    if (!popup) {
      chrome.runtime.sendMessage({
        error: true,
        message: 'Popup was blocked. To enable this feature, please allow popups for app.tempo.new in your Chrome settings.'
      });
      sendResponse({ success: false });
      return;
    }
    
    console.log('Chrome direct window.open successful');
    sendResponse({ success: true });
